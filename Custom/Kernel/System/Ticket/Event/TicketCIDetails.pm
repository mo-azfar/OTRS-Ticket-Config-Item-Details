			...
			my($Class, $Definition) = split(/::/, $ImportedCIValue, 2);
		  	...
		  	{ 
		    	...
		    	{ 
			  		...
			  		#check config item type
			  		if ($hash_ref->{Input}->{Type} eq "GeneralCatalog")
					{
						my $GeneralCatalogGet = $GeneralCatalogObject->ItemGet(ItemID =>"$XMLData->{$hash_ref->{Key}}->[$array[1]]->{Content}",);
						$Data1 .= "[$hash_ref->{Name}]: $GeneralCatalogGet->{Name}\n";
					}
					elsif ($hash_ref->{Input}->{Type} eq "Customer")
					{
						my %CustomerUserData = $CustomerUserObject->CustomerUserDataGet(User => "$XMLData->{$hash_ref->{Key}}->[$array[1]]->{Content}",);
						$Data1 .= "[$hash_ref->{Name}]: $CustomerUserData{UserFirstname} $CustomerUserData{UserLastname}\n";
					}
					elsif ($hash_ref->{Input}->{Type} eq "CustomerCompany")
					{
						my %CustomerCompanyGet = $CustomerCompanyObject->CustomerCompanyGet(CustomerID => "$XMLData->{$hash_ref->{Key}}->[$array[1]]->{Content}",);
						$Data1 .= "[$hash_ref->{Name}]: $CustomerCompanyGet{CustomerCompanyName}\n";
					}
					else
					{
		    	...
		    	elsif (scalar @new_array eq '2') #defintion 2nd level
		    	{
					...
					#check config item type
			  		if ($hash_ref->{Sub}->[0]->{Input}->{Type} eq "GeneralCatalog")
					{
						my $GeneralCatalogGet = $GeneralCatalogObject->ItemGet(ItemID =>"$XMLData->{$hash_ref->{Key}}->[$array[1]]->{$array[2]}->[$array[3]]->{Content}",);
						$Data1 .= "*$hash_ref->{Sub}->[0]->{Name}: $GeneralCatalogGet->{Name}\n";
					}
					else
					{
		    	...
		    	elsif (scalar @new_array eq '3') #defintion 3rd level
		    	{
			  		...
			  		#check config item type
			  		if ($hash_ref->{Sub}->[0]->{Sub}->[0]->{Input}->{Type} eq "GeneralCatalog")
					{
						my $GeneralCatalogGet = $GeneralCatalogObject->ItemGet(ItemID =>"$XMLData->{$hash_ref->{Key}}->[$array[1]]->{$array[2]}->[$array[3]]->{$array[4]}->[$array[5]]->{Content}",);
						$Data1 .= "**$hash_ref->{Sub}->[0]->{Sub}->[0]->{Name}: $GeneralCatalogGet->{Name}\n";
					}
					else
					{
